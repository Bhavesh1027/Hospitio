...
using HospitioApi.Core.HandleGuestRequest.Commands.UpdateGuestRequestStatus;
...
public class GuestRequestEndPoints : IEndpointsModule
{
    public AppRoute Route { get; init; } = new(
      plural__: "api/hospitio-customer/guestrequests",
      singular: "api/hospitio-customer/guestrequest");
    ...
    public RouteHandlerBuilder[] MapEndpoints(WebApplication app) => new[]
   {
        ...
        .AllowAnonymous(),
        app.MapPost($"/{Route.Singular}/updatestatus", UpdateGuestStatusRequestAsync)
        .AllowAnonymous(),
    };
    ...
    private async Task<IResult> UpdateGuestStatusRequestAsync([FromServices] IMediatorHelper mtrHlpr, ClaimsPrincipal cp, [FromBody] UpdateGuestRequestStatusIn @in, CT ct)
        => await mtrHlpr.ToResultAsync(new UpdateGuestRequestStatus(@in), ct);
}
