...
{
    public partial class ModificationSP_GETUserDetailByChatId_V4 : Migration
    {
...
BEGIN
	...
	BEGIN
		SELECT [C].[BusinessName],NULL AS[FirstName],NULL AS [LastName],[C].[Email],NULL AS [ProfilePicture],[C].[PhoneCountry],[C].[PhoneNumber],
			   ...
			   @UserType AS [UserType] ,@UserId AS [UserId], @ChatId AS [ChatId],@IsActive AS [IsActive],[C].[DeActivated], NULL AS [Status],NUll AS [CheckinDate], NULL AS [CheckoutDate], NULL AS [BlePinCode]
	...
	BEGIN
		SELECT	NULL AS [BusinessName],[U].[FirstName],[U].[LastName],[U].[Email],[U].[ProfilePicture],[U].[PhoneCountry],[U].[PhoneNumber],
				...
				@UserType AS [UserType] ,@UserId AS [UserId], @ChatId AS [ChatId],@IsActive AS [IsActive],[U].[DeActivated], NULL AS [Status],NUll AS [CheckinDate], NULL AS [CheckoutDate], NULL AS [BlePinCode]
	...
	BEGIN
		SELECT	NULL AS [BusinessName],[CG].[Firstname] AS [FirstName],[CG].[Lastname] AS [LastName],[CG].[Email],[CG].[Picture] AS [ProfilePicture],[CG].[PhoneCountry],[CG].[PhoneNumber],
				...
				@UserType AS [UserType] ,@UserId AS [UserId], @ChatId AS [ChatId],@IsActive AS [IsActive],[C].[DeActivated],
				CASE 
					WHEN (CONVERT(DATE, [CR].[CheckinDate]) <= CONVERT(DATE, GETDATE()) AND CONVERT(DATE, [CR].[CheckoutDate]) >= CONVERT(DATE, GETDATE())) 
						THEN 'In-House'
					WHEN (CONVERT(DATE, [CR].[CheckinDate]) <= CONVERT(DATE, GETDATE()) AND CONVERT(DATE, [CR].[CheckoutDate]) < CONVERT(DATE, GETDATE())) 
						THEN 'Checked-out'
				END AS [Status], [CR].[CheckinDate], [CR].[CheckoutDate],[CG].[BlePinCode]
