...
{
    public partial class SP_Modify_TicketsTicketCount : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            #region [SP_GetTotalUnreadTicketCount]
...
BEGIN
	...
	DECLARE @UserLevelId INT = 0;

    DECLARE @query NVARCHAR(MAX) = '';

	IF @UserType = 1
	BEGIN
		SET @GroupId  = (SELECT ISNULL(GroupId,0) FROM Users WHERE Id = @UserId);
		SET @UserLevelId  = (SELECT ISNULL(UserLevelId,0) FROM Users WHERE Id = @UserId);
	END
	SET @query += 'SELECT  COUNT(DISTINCT([T].[Id])) AS [TotalUnReadCount]
	FROM [dbo].[Tickets] T (NOLOCK)
		INNER JOIN [dbo].[TicketReplies] TR (NOLOCK)
			ON [T].[Id] = [TR].[TicketId]
			   ...
			   AND (
					(
							[TR].[CreatedFrom] = '+ CAST(@UserLevelId AS VARCHAR(50)) +'
		...
		WHERE  ( [T].[CustomerId] = '+ CAST(@CustomerId AS VARCHAR(50)) +' OR '+ CAST(@CustomerId AS VARCHAR(50)) +' = 0 )'

	    IF(@UserType =1)
			SET @query += ' 
				AND ([T].[CSAgentId] IS NULL AND [T].[GroupId] IS NULL AND '+ CAST(@UserLevelId AS VARCHAR(50)) +' = 1
					OR
					[T].[CSAgentId] = '+ CAST(@UserId AS VARCHAR(50)) +'
					OR
					([T].[GroupId] = '+ CAST(@GroupId AS VARCHAR(50)) +' AND [T].[CSAgentId] IS NULL)
				) '
		EXEC(@query)   					
END");
            #endregion
