...
{
    public partial class ADD_SP_SP_GetCustomerGuestJourneyDetailForSendMessage_V2 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
GO
/****** Object:  StoredProcedure [dbo].[SP_GetCustomerGuestJourneyDetailForSendMessage]    Script Date: 13-10-2023 10:33:44 ******/
SET ANSI_NULLS ON
GO
        ...
        BEGIN
            ...
            SET @query
                = ' INSERT INTO #TempTables ([Id],[CustomerId],[JourneyStep],[Uuid],[ReservationNumber],[Source],[NoOfGuestAdults],[NoOfGuestChildrens],[CheckinDate],[CheckoutDate],[IsActive],[CreatedAt],[isCheckInCompleted],[isSkipCheckIn],[Email],[Phone],[TempletMessage],[GuestId],[Buttons],[VonageTemplateId],[VonageTemplateStatus],[CustomerUserId],[CustomerWhatsAppNumber],[APPId],[PrivateKey],[TemplateName],[BussinessName],[BussinessAddress],[BookingDays],BussinessPhoneNumber,GuestName)
					SELECT DISTINCT ''' + CAST(@CustomerId AS NVARCHAR(10)) + ''',[TTR].[Id],'''+ CAST(@JourneyStep AS NVARCHAR(10))+ ''',[TTR].[Uuid],[TTR].[ReservationNumber],[TTR].[Source],[TTR].[NoOf
					Adults],[TTR].[NoOfGuestChildrens],[TTR].[CheckinDate],[TTR].[CheckoutDate],[TTR].[IsActive],[TTR].[CreatedAt],[CG].[isCheckInCompleted],[CG].[isSkipCheckIn],[CG].[Email],[CG].[PhoneNumber],'''+ CAST(@TempletMessage AS NVARCHAR(MAX))+ ''',[CG].[Id]
            ...
            BEGIN
                SET @query
                    = ' INSERT INTO #TempTables ([Id],[CustomerId],[JourneyStep],[Uuid],[ReservationNumber],[Source],[NoOfGuestAdults],[NoOfGuestChildrens],[CheckinDate],[CheckoutDate],[IsActive],[CreatedAt],[isCheckInCompleted],[isSkipCheckIn],[Email],[Phone],[TempletMessage],[GuestId],[Buttons],[VonageTemplateId],[VonageTemplateStatus],[CustomerUserId],[CustomerWhatsAppNumber],[APPId],[PrivateKey],[TemplateName],[BussinessName],[BussinessAddress],[BookingDays],BussinessPhoneNumber,GuestName)
                      ...
                      + ''',[TTR].[Uuid],[TTR].[ReservationNumber],[TTR].[Source],[TTR].[NoOfGuestAdults],[TTR].[NoOfGuestChildrens],[TTR].[CheckinDate],[TTR].[CheckoutDate],[TTR].[IsActive],[TTR].[CreatedAt],[CG].[isCheckInCompleted],[CG].[isSkipCheckIn],[CG].[Email],[CG].[PhoneNumber],'''
