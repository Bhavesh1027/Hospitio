...
{
    public partial class Modify_SP_GETUserDetailByChatId_SPs_V3 : Migration
    {
...
BEGIN
	...
	BEGIN
		SELECT	NULL AS [BusinessName],
				...
				[C].[DeActivated],
				CASE 
					WHEN dbo.GetGuestsStatus([CG].[Id]) = 1 THEN 'Expected'
					WHEN dbo.GetGuestsStatus([CG].[Id]) = 2 THEN 'In-House'
					WHEN dbo.GetGuestsStatus([CG].[Id]) = 3 THEN 'Checked-out'
				END AS [Status], 
				[CR].[CheckinDate], 
				[CR].[CheckoutDate],
				[CG].[BlePinCode]
