...
{
    public partial class ModifySP_GetTotalUnReadMessageCountV3 : Migration
    {
...
BEGIN
	...
	IF(@UserType = 3)
	BEGIN
		SELECT COUNT(DISTINCT([CM].[Id])) AS [TotalUnreadCount], 'communication' As [Type]
	...
	ELSE 
	BEGIN
		SELECT COUNT(DISTINCT([C].[Id])) AS [TotalUnreadCount], 'communication' As [Type]
		FROM [dbo].[Channels] C (NOLOCK)
			INNER JOIN [dbo].[ChannelUsers] CU 
				ON [C].[Id] = [CU].[ChannelId] 
					AND [CU].[UserId] = @UserId
