...
{
    public partial class ModifySP_GetCustomerGuestJourneyDetailForSendMessageV3 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
/****** Object:  StoredProcedure [dbo].[SP_GetCustomerGuestJourneyDetailForSendMessage]    Script Date: 31-01-2024 14:33:26 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER           PROCEDURE [dbo].[SP_GetCustomerGuestJourneyDetailForSendMessage]-- '2024-01-30 09:14:09.230',29
(
...
BEGIN
    ...
    DECLARE @JourneyStep INT,
			...
			@EmailserviceEnable BIT,
			@WebChatserviceEnable BIT,
			@customerLogoURL NVARCHAR(MAX),
			@color NVARCHAR(6),
			@SMSTwoWayCommunication BIT
...
FROM 
    Products p (NOLOCK) ,
    Modules m (NOLOCK),
    ProductModules pm (NOLOCK),
    ModuleServices ms (NOLOCK),
    ProductModuleServices pms (NOLOCK),
    Customers c (NOLOCK)
WHERE 
...
SET @WebChatserviceEnable = ( SELECT 
    CASE 
        WHEN p.IsActive = 1 AND pm.IsActive = 1 AND pms.IsActive = 1 THEN 1
        ELSE 0
    END AS Result
...
WHERE 
    ...
    pms.ProductId = p.Id AND
    pms.ProductModuleId = pm.Id AND
    ms.Name = 'Web Chat' AND
    c.ProductId = p.Id AND
	c.Id = @CustomerId)


  SET @customerLogoURL = ( select Logo from CustomerGuestsCheckInFormBuilders (NOLOCK) where CustomerId = @CustomerId )
  SET @color = ( select Color from CustomerGuestsCheckInFormBuilders (NOLOCK) where CustomerId = @CustomerId )
  SET @SMSTwoWayCommunication = (SELECT TOP(1) IsTwoWayComunication FROM Customers (NOLOCK) WHERE Id = @CustomerId)
    ...
    (
		...
		[EligibleForEmailCommunication] NVARCHAR(MAX),
		[EligibleForWebChatCommunication] NVARCHAR(MAX),
		[CustomerLogoURL] NVARCHAR(MAX),
		[CustomerColour] NVARCHAR(6),
		[SMSTwoWayCommunication] BIT
    )
    ...
    SELECT DISTINCT
		...
		[EligibleForEmailCommunication],
		[EligibleForWebChatCommunication],
		[CustomerLogoURL],
		[CustomerColour],
		[SMSTwoWayCommunication]
    FROM #TempTables
    ORDER BY ID

END");
            migrationBuilder.Sql(@"/****** Object:  StoredProcedure [dbo].[SP_GetChannelDataFromUsersDetail]    Script Date: 31-01-2024 17:59:01 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE OR ALTER     PROCEDURE [dbo].[SP_GetChannelDataFromUsersDetail]
	@UserIdTo INT,
...
BEGIN
    ...
    INNER JOIN ChannelUsers cu1 (NOLOCK) ON c.Id = cu1.ChannelId
    INNER JOIN ChannelUsers cu2 (NOLOCK) ON c.Id = cu2.ChannelId
