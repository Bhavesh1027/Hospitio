...
{
    public partial class modifySP_GetCustomers_GetLeads_V2 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
			#region GetCustomers
			migrationBuilder.Sql(@"/****** Object:  StoredProcedure [dbo].[GetCustomers]    Script Date: 05-07-2024 17:22:46 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    ...
    AS (SELECT [dbo].[Customers].[Id],
        ...
        (
			...
			WHERE LTRIM(RTRIM([dbo].[Customers].[BusinessName])) LIKE LTRIM(RTRIM(s.value)) + '%'
        )
                  )
        ORDER BY CASE
...
");
			...
			#region GetLeads
			migrationBuilder.Sql(@"/****** Object:  StoredProcedure [dbo].[GetLeads]    Script Date: 05-07-2024 17:34:56 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
BEGIN
    ...
    AS (SELECT [Id],
        ...
        (
            ...
            WHERE LTRIM(RTRIM([Company])) LIKE LTRIM(RTRIM(s.value)) + '%'
        )
                  )
        ORDER BY CASE
