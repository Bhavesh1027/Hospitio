...
{
    public partial class GetGetQuestionAnswers_SP_V4 : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
GO
/****** Object:  StoredProcedure [dbo].[GetQuestionAnswers]    Script Date: 19-06-2023 09:20:18 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
/*
	exec [dbo].[GetQuestionAnswers] '',1,10,'','',0,1
*/
    ...
    AS (SELECT [QA].[Id],
        ...
        WHERE [QA].[DeletedAt] IS NULL
			  ...
			  AND (
					  ...
					  OR (
						 @IsViewAll = 0
						 AND (CAST([QA].CreatedAt AS DATE) >= CAST(GETDATE() AS DATE) OR CAST([QA].UpdateAt AS DATE) >= CAST(GETDATE() AS DATE))
						 )
                  )
        ORDER BY CASE
                     WHEN
