...
{
    public partial class Modify_GetAllTaxiTransferData_SP_For_Add_Months_Year_Filter : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            #region GetAllTaxiTransferData
            migrationBuilder.Sql(@"GO
/****** Object:  StoredProcedure [dbo].[GetAllTaxiTransferData]    Script Date: 29/05/2024 5:55:55 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	SET @query +=   ' SELECT(SELECT [TT].[Id],
					 ...
					 BEGIN
					  SET @query += 'AND CAST([TT].[PickUpDate] AS DATE) >= '''+ CAST(CAST(@FromCreate AS DATE) AS NVARCHAR(100))+'''   '
					 END

					 IF(@ToCreate IS NOT NULL)
					 BEGIN
					  SET @query += 'AND CAST([TT].[PickUpDate] AS DATE) <= '''+ CAST(CAST(@ToCreate AS DATE) AS NVARCHAR(100))+''' '
					 END
					 ...
					 BEGIN
					   SET @query += 'AND ( MONTH([TT].[PickUpDate]) = ' + CAST(@Month AS VARCHAR(100)) + ' AND YEAR([TT].[PickUpDate]) = ' + CAST(@Year AS VARCHAR(100)) + '  AND [TT].[IsMonthlyReport] = 1  ) '
					 END
