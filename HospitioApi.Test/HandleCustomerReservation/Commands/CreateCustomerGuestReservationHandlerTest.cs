using FakeItEasy;
using Microsoft.Extensions.Logging;
using Microsoft.Extensions.Options;
using HospitioApi.Core.HandleCustomerReservation.Commands.CreateCustomerGuestReservation;
using HospitioApi.Core.HandleCustomers.Queries.GetCustomerByGuId;
using HospitioApi.Core.Options;
using HospitioApi.Core.Services.Dapper;
using HospitioApi.Core.Services.HandlerResponse;
using HospitioApi.Core.Services.SendEmail;
using HospitioApi.Data;
using Xunit;
using ThisTestFixure = HospitioApi.Test.HandleCustomerReservation.Commands.CreateCustomerGuestReservationHandlerFixure;

namespace HospitioApi.Test.HandleCustomerReservation.Commands
{
    public class CreateCustomerGuestReservationHandlerTest : IClassFixture<ThisTestFixure>
    {
        private readonly ThisTestFixure _fix;
        public CreateCustomerGuestReservationHandlerTest(ThisTestFixure fix)
        {
            _fix = fix;
        }
        [Fact]
        public async Task success()
        {
            using var db = new ApplicationDbContext(_fix.DbContextOptions, _fix.TenantService);
            var _dappar = A.Fake<IDapperRepository>();

            var actualReservationNumber = _fix.In.ReservationNumber;
            _fix.In.ReservationNumber = "2454DF";

            A.CallTo(() => _fix.sendEmail.ExecuteAsync(null, A<CancellationToken>.Ignored)).WhenArgumentsMatch(x => x.Count() > 0).Returns(true);
            A.CallTo(() => _dappar.GetSingle<CustomerByGuIdOut>(A<string>.Ignored, null, CancellationToken.None,
                System.Data.CommandType.StoredProcedure)).WhenArgumentsMatch(x => x.Count() > 0).Returns(_fix.customerResponse);

            var result = await _fix.BuildHandler(db,_dappar).Handle(new(_fix.In), CancellationToken.None);

            Assert.True(result.HasResponse);
            Assert.True(result.Response!.Message == "Create customer reservation successful.");
            _fix.In.ReservationNumber = actualReservationNumber;
        }
        [Fact]
        public async Task Not_Found_Error()
        {
            using var db = new ApplicationDbContext(_fix.DbContextOptions, _fix.TenantService);
            var _dappar = A.Fake<IDapperRepository>();

            var actualObj = _fix.customerResponse;
            _fix.customerResponse = null;
            A.CallTo(() => _dappar.GetSingle<CustomerByGuIdOut>(A<string>.Ignored, null, CancellationToken.None,
                System.Data.CommandType.StoredProcedure)).WhenArgumentsMatch(x => x.Count() > 0).Returns(_fix.customerResponse);

            var result = await _fix.BuildHandler(db, _dappar).Handle(new(_fix.In), CancellationToken.None);

            Assert.True(result.HasFailure);
            Assert.True(result.Failure!.Message == "Data not available");
            _fix.customerResponse = actualObj;  
        }
        [Fact]
        public async Task Reservation_Already_Exist()
        {
            using var db = new ApplicationDbContext(_fix.DbContextOptions, _fix.TenantService);
            var _dappar = A.Fake<IDapperRepository>();

            A.CallTo(() => _dappar.GetSingle<CustomerByGuIdOut>(A<string>.Ignored, null, CancellationToken.None,
                System.Data.CommandType.StoredProcedure)).WhenArgumentsMatch(x => x.Count() > 0).Returns(_fix.customerResponse);

            var result = await _fix.BuildHandler(db, _dappar).Handle(new(_fix.In), CancellationToken.None);

            Assert.True(result.HasFailure);
            Assert.True(result.Failure!.Message == "The customer reservation already exists.");
        }
    }
    public class CreateCustomerGuestReservationHandlerFixure : DbFixture
    {
        public HandlerResponseFactory Response { get; set; } = new HandlerResponseFactory();
        public CreateCustomerGuestReservationIn In { get; set; } = new();
        public IOptions<JwtSettingsOptions> jwtSettings { get; set; } = A.Fake<IOptions<JwtSettingsOptions>>();
        public IOptions<FrontEndLinksSettingsOptions> frontEndLinksSettings { get; set; } = A.Fake<IOptions<FrontEndLinksSettingsOptions>>();
        public ISendEmail sendEmail { get; set; } = A.Fake<ISendEmail>();
        public CustomerByGuIdOut customerResponse { get; set; } = new();
        public IOptions<HospitioApiStorageAccountOptions> hospitioApiStorageAccount { get; set; } = A.Fake<IOptions<HospitioApiStorageAccountOptions>>();
        protected override void Seed()
        {
            using var db = new ApplicationDbContext(DbContextOptions, TenantService);
            db.Database.EnsureDeleted();
            db.Database.EnsureCreated();

            var customer = CustomerFactory.SeedSingle(db);
            var customerReservation = CustomerReservationFactory.SeedSingle(db, customer.Id);
            var customerRoomName = CustomerRoomNamesRepository.SeedSingle(db,customer.Id);
            var customerGuest = CustomerGuestFactory.SeedSingle(db, customerReservation.Id);

            In.UserCode = customer.Guid;
            In.ReservationNumber = customerReservation.ReservationNumber;
            In.LocationCode = customerRoomName.Guid;
            In.AppAccessCode = customerGuest.AppAccessCode;
            In.BluetoothPinCode = customerGuest.BlePinCode;

            var checkInDate = customerReservation.CheckinDate;
            var checkoutDate = customerReservation.CheckoutDate;

            In.ArrivalDate = DateOnly.FromDateTime((DateTime)checkInDate).ToString();
            In.ArrivalTime = TimeOnly.FromDateTime((DateTime)checkInDate).ToString();
            In.DepartureDate = TimeOnly.FromDateTime((DateTime)checkoutDate).ToString();
            In.DepartureTime = TimeOnly.FromDateTime((DateTime)checkoutDate).ToString();

            customerResponse.Id = customer.Id;
            customerResponse.Guid = customer.Guid;
            customerResponse.BusinessStartTime = customer.BusinessStartTime;
            customerResponse.BusinessName = customer.BusinessName;
            customerResponse.BusinessTypeId = customer.BusinessTypeId;
            customerResponse.TimeZone = customer.TimeZone;
            customerResponse.NoOfRooms = customer.NoOfRooms;

            jwtSettings.Value.JwtPemPrivateKey = @"-----BEGIN PRIVATE KEY----------END PRIVATE KEY-----";

            frontEndLinksSettings.Value.GuestPortal = "http://localhost:3002/";

        }
        public CreateCustomerGuestReservationHandler BuildHandler(ApplicationDbContext db,IDapperRepository _dapper) =>
               new(db, Response,_dapper, jwtSettings, frontEndLinksSettings, sendEmail, hospitioApiStorageAccount);
    }
}
